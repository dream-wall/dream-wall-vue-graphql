<template>
    <div class="dw-dream">
      <div class="dw-dream__hero">
        ...
        <div class="dw-dream__tags">
          <span class="dw-dream__lastest"
            ...
            @click="getDreamsWithGraphql({page: 1, row, sort: 'last'})">最新</span>
          <span class="dw-dream__hot"
            :class="{ ' dw-dream__tags--active': sort === 'pop' }"
            @click="getDreamsWithGraphql({page: 1, row, sort: 'pop'})">热门</span>
        </div>
      </div>
      <template v-if="dreams.length>0 && !loading">
        ...
        <div v-if="!end || more" class="dw-dream__more" @click="getDreamsWithGraphql({page: ++page, row, sort: sort})">
            <template v-if="!more">
              体验更多梦想
            </template>
...
export default {
  ...
  methods: {
    ...
    getDreamsWithGraphql ({page = 1, row = 12, sort = 'last'} = {}) {
      if (page === 1 && sort === this.sort) return // 如果请求的是第一页 && 请求的是同一类别
      if (this.end && sort === this.sort) return // 如果请求到最后一页 && 请求的是同一类别（最新或最热）
      if (sort !== this.sort) { // 请求的是不同类别（说明做了切换最新和最热）
        this.sort = sort
      ...
      this.$apollo.addSmartQuery('dreams', {
        query: gql`query dreams($current_page: Int!, $page_size: Int!, $sort: String!) {
          dreams(current_page: $current_page, page_size: $page_size, sort: $sort) {
            code
            result {
              count
              rows {
        ...
        result ({data, loader, networkStatus}) {
          ...
          if (page === 1) {
            ...
            setTimeout(_ => {
              this.dreams = [...this.dreams, ...res.result.rows]
              this.more = false
            }, 1000)
          }
        }
